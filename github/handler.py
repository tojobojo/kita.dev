...
from github.templates import ACKNOWLEDGEMENT_MSG, STOP_MSG_TEMPLATE, FAILURE_MSG_TEMPLATE
...
class WebhookHandler:
    ...
    def handle_event(self, event_type: str, payload: Dict[str, Any]):
        ...
        if not command:
            if body.strip().lower().startswith("@kita"):
                 ...
                 if self.gh.authenticate(installation_id):
                     self.gh.post_comment(issue_number, "Command not recognized. Try '@kita help'.", repo_full_name)
            return
        ...
        try:
            ...
            # Configure Git
            subprocess.run(["git", "config", "user.name", "Kita Bot"], cwd=work_dir, check=True)
            subprocess.run(["git", "config", "user.email", "bot@kita.dev"], cwd=work_dir, check=True)
            ...
            if command.action == "IMPLEMENT":
                ...
                self._handle_completion(issue_number, final_state, work_dir, branch_name, repo_full_name)
            ...
            self.gh.post_comment(issue_number, f"💥 **System Error**: {str(e)}", repo_full_name)
        finally:
            # Cleanup
            shutil.rmtree(work_dir)
            logger.info(f"Cleaned up workspace: {work_dir}")

    def _handle_completion(self, issue_number: int, state: AgentState, work_dir: str, branch_name: str, repo_full_name: str):
        if state == AgentState.COMPLETED:
            # Commit all changes made by agent
            self.pr_builder.commit_changes(work_dir, [], f"Fix for issue #{issue_number}")
            ...
            if self.pr_builder.push_branch(work_dir, branch_name):
                ...
                if res.success:
                    self.gh.post_comment(issue_number, f"✅ **Task Completed!**\n\nPR Opened: {res.pr_url}", repo_full_name)
                else:
                    self.gh.post_comment(issue_number, f"⚠️ **Task Completed but PR Failed**\n\nError: {res.error}", repo_full_name)
            else:
                self.gh.post_comment(issue_number, "⚠️ Failed to push changes to branch.", repo_full_name)
            
        elif state == AgentState.STOPPED_SAFE:
            self.gh.post_comment(issue_number, STOP_MSG_TEMPLATE.format(reason="Safety check triggered."), repo_full_name)
            
        elif state == AgentState.STOPPED_ERROR:
            self.gh.post_comment(issue_number, FAILURE_MSG_TEMPLATE.format(
