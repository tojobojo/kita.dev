...
class BudgetTracker:
    ...
    def consume(self, tokens: int) -> bool:
        ...
        if self.remaining_budget < 0:
            logger.warning(
                f"Budget exhausted! Used {self.total_used}/{self.initial_budget}"
            )
            return False
        return True
    ...
    def get_usage_summary(self) -> dict:
        ...
        return {
            ...
            "budget_utilization": (
                self.total_used / self.initial_budget if self.initial_budget > 0 else 0
            ),
        }
...
class LiteLLMClient(LLMClient):
    ...
    API keys can be passed directly (BYOK) or loaded from environment variables.
    """

    def __init__(
        self, model: str = "gpt-4-turbo", api_key: str = None, api_base: str = None
    ):
        """
        ...
        Args:
            ...
            api_key: Optional API key (BYOK). If not provided, uses env vars.
            api_base: Optional API base URL for custom endpoints.
        """
        super().__init__()
        self.model = model
        self.api_key = api_key  # BYOK support
        ...
        try:
            ...
            logger.error(
                "litellm package not installed. Please run `pip install litellm`."
            )
            self.litellm = None
    ...
    def complete(self, messages: List[Dict[str, str]], temperature: float = 0.0) -> str:
        ...
        try:
            # Build kwargs with optional BYOK params
            kwargs = {
                "model": self.model,
                "messages": messages,
            ...
            # BYOK: Pass API key directly if provided
            if self.api_key:
            ...
            if usage:
                ...
                logger.info(
                    f"LLM tokens used: {usage.total_tokens} (in: {usage.prompt_tokens}, out: {usage.completion_tokens})"
                )
