...
class EventDispatcher:
    ...
    def dispatch(self, event: WebhookEvent) -> DispatchResult:
        ...
        if not self.router.is_supported(event):
            logger.info(
                f"Ignoring unsupported event: {event.event_type}/{event.action}"
            )
        ...
        logger.info(
            f"Dispatching event from {context['sender']} on {context['repository']['full_name']}"
        )
        ...
        if event.event_type == "issue_comment":
            ...
            return DispatchResult(
                handled=False, job_id=None, message=f"No handler for {event.event_type}"
            )

    def _handle_issue_comment(
        self, event: WebhookEvent, context: Dict[str, Any]
    ) -> DispatchResult:
        ...
        if not comment_body.strip().lower().startswith("@kita"):
            return DispatchResult(
                handled=False, job_id=None, message="Comment does not mention @kita"
            )
    ...
    def _handle_issue(
        ...
        if "kita" not in issue_labels and "kita-auto" not in issue_labels:
            return DispatchResult(
                handled=False, job_id=None, message="Issue does not have kita label"
            )
    ...
    def _handle_pr_comment(
        ...
        if not comment_body.strip().lower().startswith("@kita"):
            return DispatchResult(
                handled=False, job_id=None, message="PR comment does not mention @kita"
            )
