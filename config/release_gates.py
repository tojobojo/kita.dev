...
class ReleaseGateKeeper:
    ...
    def check_gates(self, metrics: MetricsSummary) -> GateResult:
        ...
        # 1. Completion Rate
        if metrics.total_tasks > 0 and metrics.completion_rate < self.MIN_COMPLETION_RATE:
            # Check if stops justify it? 
            # Appendix C says <50% is unusable.
            passed = False
            reasons.append(f"Completion rate {metrics.completion_rate:.2f} below minimum {self.MIN_COMPLETION_RATE}")
        ...
        if metrics.total_tasks > 20: # Only enforce after meaningful sample
            if metrics.stop_rate < self.MIN_STOP_RATE:
                passed = False
                reasons.append(f"STOP rate {metrics.stop_rate:.2f} too low (reckless). Min {self.MIN_STOP_RATE}")
                ...
                passed = False
                reasons.append(f"STOP rate {metrics.stop_rate:.2f} too high (conservative). Max {self.MAX_STOP_RATE}")

        # 3. Retry Rate (Efficiency/Intelligence)
        if metrics.retry_rate > self.MAX_RETRY_RATE:
            passed = False
            reasons.append(f"Retry rate {metrics.retry_rate:.2f} exceeds limit {self.MAX_RETRY_RATE}")
