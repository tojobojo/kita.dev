    ...
    def validate_task(self, task: str) -> ValidationResult:
        ...
        for pattern in self.VAGUE_PATTERNS:
            if re.match(pattern, task_lower, re.IGNORECASE):
                blockers.append(f"Task is too vague: '{task}'. Please provide specific requirements.")
                
        # Check for scope expansion
        for pattern in self.SCOPE_EXPANSION_PATTERNS:
            if re.search(pattern, task_lower, re.IGNORECASE):
                blockers.append(f"Task implies scope expansion. Please narrow the scope.")
                
        # Check for unsafe operations
        for pattern in self.UNSAFE_PATTERNS:
            if re.search(pattern, task_lower, re.IGNORECASE):
                blockers.append(f"Task contains potentially unsafe operation. Requires manual review.")
        
        # Check task length (too short = likely vague)
        if len(task.split()) < 3:
            warnings.append("Task description is very short. Consider adding more detail.")
            
        # Check task length (too long = might be multiple tasks)
        if len(task.split()) > 100:
            warnings.append("Task description is very long. Consider breaking into smaller tasks.")
...
class PlanValidator:
    ...
    def validate_plan(self, steps: List[dict]) -> ValidationResult:
        ...
        if len(steps) == 0:
            ...
            blockers.append(f"Plan has too many steps ({len(steps)}). Maximum is {self.MAX_STEPS}.")
        ...
        for step in steps:
            ...
            if action_type == 'COMMAND':
                if any(danger in target.lower() for danger in ['rm -rf', 'sudo', 'chmod 777']):
                    blockers.append(f"Step {step_id}: Dangerous command detected")
                    
            # Check for path traversal in EDIT
            if action_type == 'EDIT':
                if '..' in target:
                    blockers.append(f"Step {step_id}: Path traversal detected in target")
