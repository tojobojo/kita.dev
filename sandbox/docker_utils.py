...
def ensure_sandbox_image(image_name: str = "kita-sandbox:latest"):
    ...
    if not os.path.exists(dockerfile_path):
        logger.error(f"Dockerfile not found at {dockerfile_path}. Cannot build sandbox.")
    ...
    try:
        ...
        logger.error(f"Failed to build sandbox image: {e.stderr.decode() if e.stderr else 'Unknown error'}")
