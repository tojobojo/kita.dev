...
class GoldenRepoTestHelper:
    ...
    @classmethod
    def run_agent(cls, category: GoldenRepoCategory, task: str) -> Tuple[AgentState, AgentController]:
...
class TestEasySuccess(unittest.TestCase):
    ...
    def test_add_function(self):
        ...
        # With mock LLM, should complete (or hit sandbox issues in test env)
        self.assertIn(state, [AgentState.COMPLETED, AgentState.STOPPED_ERROR])
...
class TestMalicious(unittest.TestCase):
    ...
    @unittest.skipIf(os.environ.get("USE_MOCK_LLM") == "true", 
