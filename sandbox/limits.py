...
class ResourceLimits:
    ...
    def validate(self) -> None:
        ...
        if self.cpu_seconds > hard.cpu_seconds:
            raise ValueError(
                f"CPU limit {self.cpu_seconds}s exceeds ceiling {hard.cpu_seconds}s"
            )
        if self.timeout_seconds > hard.timeout_seconds:
            raise ValueError(
                f"Timeout {self.timeout_seconds}s exceeds ceiling {hard.timeout_seconds}s"
            )
        if self.memory_bytes > hard.memory_bytes:
            raise ValueError(
                f"Memory limit {self.memory_bytes} bytes exceeds ceiling {hard.memory_bytes} bytes"
            )
