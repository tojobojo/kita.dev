...
class AgentController:
    ...
    def __init__(self, api_key: str = None, model: str = None, api_base: str = None):
        ...
        Args:
            ...
            api_base: Optional custom API endpoint (e.g., Azure OpenAI)
        """
        self.state_machine = StateMachine()
        self.planner = Planner(api_key=api_key, model=model, api_base=api_base)
        ...
        try:
            ...
            if not validation.passed:
                ...
                self.state_machine.transition_to(
                    AgentState.NORMALIZED, "Task normalized (with warnings)"
                )
                self.state_machine.transition_to(
                    AgentState.STOPPED_SAFE,
                    f"Task validation failed: {validation.blockers}",
                )
            ...
            self.state_machine.transition_to(
                AgentState.CONTEXT_BUILDING, "Indexing repository"
            )
            repo_context = ""
            try:
                ...
                if not lang_result.supported:
                    ...
                    self.state_machine.force_error_stop(
                        f"Unsupported language: {lang_result.reason}"
                    )
                    return AgentState.STOPPED_ERROR

                logger.info(
                    f"Detected language: {lang_result.primary_language} (confidence: {lang_result.confidence:.2f})"
                )

                # Index repository
                repo_context = self.indexer.index(repo_path)
                self.state_machine.transition_to(
                    AgentState.CONTEXT_READY, "Context indexed"
                )
            ...
            try:
                ...
                self.state_machine.transition_to(
                    AgentState.PLAN_VALIDATED, "Plan generated and validated"
                )
            except Exception as e:
                logger.error(f"Planning failed: {e}")
                self.state_machine.transition_to(
                    AgentState.STOPPED_SAFE, f"Planning failure: {e}"
                )
                return AgentState.STOPPED_SAFE

            # 5. Execution Loop
            self.state_machine.transition_to(
                AgentState.EXECUTING_STEP, "Starting execution loop"
            )
            ...
            for step in self.current_plan.steps:
                ...
                if self.state_machine.current_state != AgentState.EXECUTING_STEP:
                    if self.state_machine.current_state == AgentState.RETRYING:
                        self.state_machine.transition_to(
                            AgentState.EXECUTING_STEP, "Retrying step"
                        )
                    else:
                ...
                if result.success:
                    self.state_machine.transition_to(
                        AgentState.STEP_COMPLETED, f"Step {step.id} success"
                    )
                    ...
                    self.state_machine.transition_to(
                        AgentState.REFLECTING, "Analyzing failure"
                    )
                    ...
                    if decision.decision == Decision.RETRY:
                        self.state_machine.transition_to(
                            AgentState.RETRYING, decision.reason
                        )
                        ...
                        # Let's say we retry.
                        self.state_machine.transition_to(
                            AgentState.EXECUTING_STEP, "Retrying execution"
                        )
                        # (In this skeleton, we won't infinite loop, just move on or retry logic)
                        continue
                    else:
                        self.state_machine.transition_to(
                            AgentState.STOPPED_SAFE, decision.reason
                        )
                        return AgentState.STOPPED_SAFE
            ...
            if self.state_machine.current_state == AgentState.EXECUTING_STEP:
                self.state_machine.transition_to(
                    AgentState.STEP_COMPLETED, "All steps executed"
                )

            if self.state_machine.current_state == AgentState.STEP_COMPLETED:
                self.state_machine.transition_to(
                    AgentState.TESTING, "Running final verification"
                )
                ...
                result = sandbox.run(
                    test_cmd, repo_path, limits=ResourceLimits(timeout_seconds=300)
                )

                tests_passed = result.exit_code == 0
                logger.info(
                    f"Verification finished. Exit code: {result.exit_code}. Output: {result.stdout[:200]}..."
                )

                if tests_passed:
                    self.state_machine.transition_to(
                        AgentState.TESTS_PASSED, "All tests passed"
                    )
                    self.state_machine.transition_to(
                        AgentState.COMPLETED, "Task successfully finished"
                    )
                else:
                    self.state_machine.transition_to(
                        AgentState.TESTS_FAILED,
                        f"Verification failed: {result.stderr[:100]}",
                    )
                    self.state_machine.transition_to(
                        AgentState.REFLECTING, "Analyzing test failure"
                    )
                    # ... Reflection loop could happen here too ...
                    self.state_machine.transition_to(
                        AgentState.STOPPED_SAFE, "Tests failed after execution"
                    )
