...
class SandboxExecutor:
    ...
    def run(
        self, command: str, repo_path: str, limits: ResourceLimits = ResourceLimits()
    ) -> CommandResult:
        """
        ...
        docker_cmd = [
            ...
            "--network",
            "none",  # Bible V Section 4: Network Isolation
            ...
            "/workspace",
            "-v",
            f"{repo_path}:/workspace",  # Bind mount the repository
            self.IMAGE_NAME,
            "/bin/bash",
            "-c",
            command,
